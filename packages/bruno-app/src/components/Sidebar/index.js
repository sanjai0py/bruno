...
const Sidebar = () => {
  ...
  const appUpdateAvailable = useSelector((state) => state.app.appUpdateAvailable);
  ...
  return (
    <StyledWrapper className="flex relative h-screen">
      <aside>
        ...
        {appUpdateModalOpen && <UpdaterModal onClose={() => setAppUpdateModalOpen(false)} updateNow={triggerUpdate} />}
